...
```{r}
## Subset dt1 to only include available (random) points
# convert dt1 to a data frame and drop geom column (remove spatial data)
# dt1df <- as.data.frame(dt1)
# dt1df <- dt1df %>% select(-geom)
# 
# dt1a <- dt1df %>% dplyr::filter(used == 0) 
# dim(dt1a)
# # 14982    49

### Now to  have nonzero values: 
dt1df <- as.data.frame(dt1)
dt1df <- dt1df %>% select(-geom)
...
dt1a$idvar <- paste0("ID-", 1:nrow(dt1a)) 
headings <- dt1a$idvar

## Now only includeLiDAR variables with IDvar
names(dt1a)

dt2a <- dt1a %>% dplyr::select(c(
  Hmax, Hmean, Hsd, Hcv, Hskewness, 
  H10, H20, H30, H40,  H50, H60, H70, H80, H90, H95, 
  D0, D1, D2, D3, D4, D5, D6, D7, D8, D9, 
  gap3.n, gap3.mean.area,  gap3.sd.area,  gap3.sum.area,  
...
### "Whereas PCA calculates the synthetic axes that minimize the total error sum of squares for a linear combination of the response variables, RDA calculates the synthetic axes that have the best linear combination of the response variables with the provided explanatory variables." 
...
################## Now do it once more if used and available points together
...
meta <- dt1 %>% dplyr::select(c(used, Species, idvar)) # for now, do not include bat species or ID

mat1 <- dt2. 
trait.pca1 <- rda(mat1, scale = TRUE) 


biplot(trait.pca1) # simple plot 
screeplot(trait.pca1) # how much of each axis describes the variation in the data 

summary(trait.pca1)

pca.summary1 <- summary(trait.pca1)

trait.loadings1 <- as.data.frame(pca.summary1$species) %>% 
  mutate(trait = rownames(.))  
head(trait.loadings1)

rownames(meta) <- meta$idvar
meta$gen.spe <- meta$idvar
meta$used <- as.factor(meta$used)

sp.scores1 <- as.data.frame(pca.summary1$sites) %>% 
  mutate(gen.spe = rownames(.)) %>% inner_join(meta) # now used data is there 

cols <- c("0" = "gray", "1" = "red")
col2 <- c("MBRA" = "skyblue", "MMYS" = "darkblue")

## PC1 and PC2 - used is color 
pca.ggplot <- ggplot(data = sp.scores1, aes(x = PC1, y = PC2)) +
...
## PC1 and PC3
pca.ggplot <- ggplot(data = sp.scores1, aes(x = PC1, y = PC3)) +
  geom_point(aes(color = used), alpha = 0.3) +
  scale_color_manual(values = cols)+ 
  geom_segment(data = trait.loadings, 
               aes(x = 0, xend = PC1/2, y = 0, yend = PC2/2), lwd = 0.1) + 
