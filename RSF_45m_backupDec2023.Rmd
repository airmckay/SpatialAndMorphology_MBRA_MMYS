...
openareas <- dt2df %>% filter(ttops.n <= 100)
# 8339 obs
8339/16483
## If I use 5 trees as the cut off then we get 
# 14%
## If I use 100 trees, then I get 50% (for all points)
# How many of these open area observations are actual used points?
openareasused <- dt2dfused %>% filter(ttops.n <= 100)
# 715 
715/1502
# Also about 50% 
openareasused <- dt2dfused %>% filter(ttops.n <= 50)
# 428 
428/1502
# 28% 
table(openareasused$Species)
...
summary(mbraused$ttops.n)
   ...
   #  0.0    31.0    82.0   102.1   129.0   447.0  
summary(mbraused$ttops.mean.height)
   # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
   # 0.00   13.31   16.17   16.30   19.08   31.58 
## Most used points are in forests (vegetation more than 13 m tall) where there are at least 31 trees (sparse forest) 
...
dfopen1 <- dt2dfused %>% filter(ttops.n <75,
                                gap3.mean.area > 5500) 
...
# Species:water.distance    1 9545.9 *** 
############ Suggests to drop water.distance*Species 
...
p2 <- ggplot(pred, aes(gap3.sd.area, PP)) + 
  geom_line(aes(col = factor(Species)), linewidth = 1.5) + 
  geom_ribbon(aes(x = gap3.sd.area, ymin = LL, ymax = UL, 
              fill = factor(Species)), alpha = 0.4) +
  facet_wrap(~Species) +
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.01,
  ...
  scale_colour_manual(values = 
                      ...
                      aesthetics = c("fill", "colour"), 
                      name = "Bat species") + 
  xlab ("Standard deviation of area of gaps within 45 m of a location") +
  ylab ("Predicted probability of use") +
  xlab ("gap3.sd.area") +
  ylab ("Probability of use") +
  theme(text = element_text(size=20))
...
## Bit curious  about how a massive model of all the variables would behave in terms of R2 
```{r}
# 
# bigfit.test = glmer(used ~  Species + 
#                   gap3.mean.area + gap3.sd.area + 
#                   gap3.n + gap3.sum.area +
#                   gap5.mean.area + gap5.sd.area +
#                   gap5.n + gap5.sum.area +
#                   ttops.n + ttops.mean.height + ttops.sd.height +
#                   D0 + D1 + D2 + D3 + D4 + 
#                   D5 + D6 + D7 + D8 + D9 +
#                   H10 + H20 + H30 + H40 +  H50 + H60 +
#                   H70 + H80 + H90 + H95 +
#                   Hsd + Hmax + Hmean + Hcv + Hskewness + Hkurt +
#                   water.distance +
#                   gap3.mean.area*Species + gap3.sd.area*Species + 
#                   gap3.n*Species + gap3.sum.area*Species +
#                   gap5.mean.area*Species + gap5.sd.area*Species +
#                   gap5.n*Species + gap5.sum.area*Species +
#                   ttops.n*Species + 
#                   ttops.mean.height*Species + 
#                   ttops.sd.height*Species +
#                   D0*Species + D1*Species + D2*Species + 
#                   D3*Species + D4*Species + D5*Species + 
#                   D6*Species + D7*Species + 
#                   D8*Species + D9*Species +
#                   H10*Species + H20*Species + H30*Species + 
#                   H40*Species +  H50*Species + H60*Species +
#                   H70*Species + H80*Species + 
#                   H90*Species + H95*Species +
#                   Hsd*Species + Hmax*Species + Hmean*Species + 
#                   Hcv*Species + Hskewness*Species + Hkurt*Species +
#                   water.distance*Species +
#                   (1|BatID), data=dt1,
#               family=binomial(link="logit"),nAGQ = 0)
# 
# sumbig <- summary(bigfit.test)$r.squared # Cannot get the R2 for this model 
# print(bigfit.test, correlation=TRUE)
# bigfit.test
# 
# library(MuMIn)
# r.squaredGLMM(bigfit.test)